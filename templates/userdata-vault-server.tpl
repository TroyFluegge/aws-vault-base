...
seal "awskms" {
  region     = "${tpl_vault_aws_region}"
  kms_key_id = "${tpl_vault_kms_unseal_key}"
}
...
PermissionsStartOnly=true
ExecStartPre=/sbin/setcap 'cap_ipc_lock=+ep' /usr/local/bin/vault
ExecStart=/usr/local/bin/vault server -config /etc/vault.d/vault.hcl \$OPTIONS
ExecReload=/bin/kill -HUP \$MAINPID
KillSignal=SIGTERM
User=vault
Group=vault
...
##
## This entire section doesn't account for any other default users beside the "ubuntu" user
## I'll fix this later
%{ for address, name in tpl_vault_node_address_names  ~}
  echo "${address} ${name}" | sudo tee -a /etc/hosts
%{ endfor ~}

%{ if tpl_vault_license != "oss" }
  logger "Applying autoload license to Enterprise Vault"
  echo ${tpl_vault_license_blob} > /etc/vault.d/license.hclic
  echo "license_path = \"/etc/vault.d/license.hclic\"" >> /etc/vault.d/vault.hcl
  echo "Applied license to (${tpl_vault_node_name})"
%{ endif }
...
%{ if tpl_vault_count_index != 0 }
  logger "Joining Raft Cluster"
  until vault operator raft join http://${tpl_vault_active_node}:8200; do
    echo "Waiting for primary node (${tpl_vault_active_node})"
    sleep 15
  done
%{ endif }
